...
- Logging is one tap: the **I had this now** button (or `L` shortcut) creates a log entry with a timestamp. Notes aren’t captured yet, keeping the flow frictionless.

## 🔐 Global Settings & Privacy

- Problems are private by default; there’s no timeline sharing or public feed. Everything lives in your workspace.

## 📊 Widgets & Dashboards

- No dedicated Problems widget today. The page itself is the primary view—use the log counts to gauge priority.
...
- Problems don’t post to the timeline or award leaderboard points—this is purely an internal tracker.

## 🔗 Integrations & Dependencies

- Problem logs are stored separately (see `ProblemLog` records in the database). Deleting a problem removes its logs.
- There are no downstream integrations yet; analytics rely on the per-problem log count surfaced on each card.
