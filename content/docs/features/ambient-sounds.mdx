...
- Enable the feature in `Settings → Features → Ambient Sounds`. Once it’s on, you’ll see the music note widget in the sidebar and the My Day dashboard.
...
- Ambient audio stays local to your device, so there’s nothing to configure in privacy or timelines.
...
- Spotlight includes **“Ambient sounds”** so you can open the manager without touching the mouse (feature flag must be enabled).
- The Music widget’s action button and the sidebar widget both open the manager, making the control panel two clicks away from anywhere.

## ⌨️ Keyboard Shortcuts

- `Alt + A`: open the Ambient Sound dialog from anywhere.
- `Alt + M`: toggle the entire ambient mix on/off.
- `Cmd/Ctrl + K`: open Spotlight to jump to the **Ambient sounds** action without leaving your current context.
