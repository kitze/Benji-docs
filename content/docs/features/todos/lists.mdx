...
- Lists ride on the Todos feature, so first enable `Settings → Features → Todos` and keep the Todos sidebar visible on desktop or mobile.
...
- Lists can be created from Alfred (`Cmd/Ctrl + K` → “Create a new todo list”), the sidebar “+” button, or the list context menu (use **Create child list** to nest).
...
- Lists respect the global Todo settings (badge counts, subtask behaviour, points display).
...
- Alfred (`Cmd/Ctrl + K`) exposes “Create a new todo list”, “Create multiple new todos”, “Convert todo list to project”, and quick navigation to specific lists. Spotlight search surfaces list names so you can jump directly into a list or its sharing manager without hunting through the sidebar.
- The multi-select toolbar inside Todos includes a **Move to list** popover—select a batch of tasks, click the list icon, search for the target list, and they’ll adopt the new context (and its default assignee) instantly.

## ⌨️ Keyboard Shortcuts

- Lists inherit the global todo shortcuts: `q` for quick add, `Cmd/Ctrl + K` for Alfred, and quick menu hotkeys (`D`, `P`, `T`, `J`, `L`, `A`).
